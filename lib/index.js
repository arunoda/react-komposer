...
export function composeWithTracker(reactiveFn) {
...
export function composeWithPromise(fn) {
  const onPropsChange = (props, onData) => {
    const promise = fn(props);
    invariant(
      (typeof promise.then === 'function') &&
      (typeof promise.catch === 'function'),
      'You must return a promise from the callback of `composeWithPromise`'
    );
    ...
    promise
      .then(data => {
        invariant(
          typeof data === 'object',
          'You must return a plain object from the promise'
        );
