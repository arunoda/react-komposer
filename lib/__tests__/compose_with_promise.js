...
describe('composeWithPromise', () => {
  it('should throw error when return something else than a promise', () => {
    ...
    expect(run).to.throw(/You must return a promise/);
  });
  ...
  it('should get data and handle them', done => {
    ...
    setTimeout(() => {
      expect(el.state('_fnData').payload.name).to.be.equal('arunoda');
      done();
    }, 0);
  });
  ...
  it('should throw an error if data is not an object', done => {
    ...
    // After promise resolved, it'll show the data.
    setTimeout(() => {
      expect(el.state('_fnData').payload).to.be.equal(undefined);
      done();
    }, 0);
  });
  ...
  it('should handle errors', done => {
    ...
    setTimeout(() => {
      expect(el.state('_fnData').error.message).to.be.equal('aa');
      done();
    }, 0);
  });
});
